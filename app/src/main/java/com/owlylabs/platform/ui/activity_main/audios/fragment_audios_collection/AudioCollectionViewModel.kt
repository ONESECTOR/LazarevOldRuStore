...
import com.owlylabs.platform.ui.audios.fragment_audios_collection.audio_list.AudioCollectionListItemAbstract
import com.owlylabs.platform.ui.audios.fragment_audios_collection.audio_list.AudioCollectionListItemSingleAudio
import com.owlylabs.platform.ui.audios.fragment_audios_collection.audio_list.AudioCollectionListItemSubtitle

class AudioCollectionViewModel(repository: AbstractLocalRepository, sectionId: Int) : ViewModel() {

    var sectionData = MediatorLiveData<SectionData>()
    private var hiddenSectionData: LiveData<SectionData>

    var listLiveData = MediatorLiveData<ArrayList<AudioCollectionListItemAbstract>>()
    private var hiddenListLiveData: LiveData<ArrayList<AudioCollectionListItemAbstract>>
    ...
    init {
        ...
        hiddenListLiveData = LiveDataReactiveStreams.fromPublisher(
            repository
                .getAudiosBySectionIdFlowable(sectionId)
                .map { listOfBooks ->
                    val tabListItemAbstract = ArrayList<AudioCollectionListItemAbstract>()
                    val sectionData = repository.getSectionByIdBlocking(sectionId)
                    tabListItemAbstract.add(AudioCollectionListItemSubtitle(sectionData))
                    listOfBooks.forEach { bookd ->
                        tabListItemAbstract.add(AudioCollectionListItemSingleAudio(sectionData, bookd))
                    }
    ...
    class Factory(val repository: AbstractLocalRepository, val sectionId: Int) :
        ...
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            return AudioCollectionViewModel(
                repository,
                sectionId
            ) as T
        }
